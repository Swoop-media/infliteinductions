...
**Update (Aug 2026):** the live-DB policy sweep also removed the learner SELECT policy on `quizzes` itself — learners saw 0 quiz rows and therefore 0 questions ("Quiz Ready, No Questions"), while creators/admins were unaffected. Restored by migration 027. Lesson: after any policy sweep, verify access as a *plain learner* (admin.generateLink → verifyOtp with anon key), not just via service role.
