...
/**
 * Notes:
 * - Keeps your existing logic and table names (assignment_progress, etc.).
 * - Adds tag revalidation: revalidateTag(`course:${course_id}:meta`) after writes.
 * - Minor parallelism added when checking “all modules completed”.
 * - Returns explicit no-store headers for safety with user-specific responses.
 */
  ...
  try {
    ...
    if (!assignmentId || !moduleId) {
      ...
      return NextResponse.json(
        { error: "Missing assignmentId or moduleId" },
        { status: 400 }
      );
    }

    // Use admin client to bypass RLS for verification/upserts
    const adminClient = supabaseAdmin();

    // Fetch assignment (existence + course link)
    ...
    // Verify user can manage this assignment (trainee OR trainer/assessor for this course)
    ...
    if (!isTrainee && !isTrainerOrAssessor) {
      ...
      return NextResponse.json(
        { error: "Not authorized to manage this assignment" },
        { status: 403 }
      );
    }
    ...
    if (completed) {
      ...
      // Try to complete the overall assignment (keep your existing RPC)
      try {
        const rpcParams: any = { p_assignment_id: assignmentId };
        await adminClient.rpc("try_complete_assignment", rpcParams);
      } catch (error) {
        console.warn("Failed to run try_complete_assignment RPC:", error);
      }

      // Check if all modules are completed (parallelize reads)
      const [allModulesRes, completedProgressRes] = await Promise.all([
        adminClient
          .from("course_modules")
          .select("id")
          .eq("course_id", typedAssignmentCheck.course_id),
      ...
      const allModuleIds = allModulesRes.data?.map((m) => m.id) || [];
      const completedModuleIds = completedProgressRes.data?.map((p) => p.module_id) || [];
      const allModulesCompleted =
        allModuleIds.length > 0 && allModuleIds.every((id) => completedModuleIds.includes(id));
      ...
      if (allModulesCompleted) {
        ...
        if (currentAssignment && currentAssignment.assignment_status !== "completed") {
          ...
          if (!completeError) {
            ...
            // Authorization derivation remains as-is
            ...
            if (authCourses && authCourses.length > 0) {
              for (const authCourse of authCourses) {
                ...
                const allAuthCoursesCompleted =
                  completedCourses?.length === courseIds.length && courseIds.length > 0;
    ...
    // 🔄 Revalidate cached course meta so the streamed page picks up changes quickly
    if (typedAssignmentCheck?.course_id) {
      try {
        revalidateTag(`course:${typedAssignmentCheck.course_id}:meta`);
      } catch (e) {
        // non-fatal
