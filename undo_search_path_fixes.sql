-- UNDO SCRIPT: Remove search_path settings from functions
-- Only run this if you want to revert the security fixes

-- 1. Revert upsert_learner_document
...
RETURNS uuid
LANGUAGE plpgsql
-- REMOVED: SET search_path = public, pg_catalog
AS $function$
DECLARE
    v_document_id UUID;
...
BEGIN
  SELECT json_build_object(
    'course', (SELECT row_to_json(c) FROM courses c WHERE c.id = p_course_id),
    'modules', COALESCE((SELECT json_agg(m) FROM course_modules m WHERE m.course_id = p_course_id ORDER BY m.order_index), '[]'::json),
    'requirements', COALESCE((SELECT json_agg(r) FROM onsite_requirements r WHERE r.module_id IN (SELECT id FROM course_modules WHERE course_id = p_course_id)), '[]'::json)
  ) INTO result;
...
-- 3. Revert save_learner_document
...
-- 4. Revert trigger_check_authorization_on_course_complete
